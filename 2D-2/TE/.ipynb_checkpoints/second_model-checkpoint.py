...
class second_model(object):
    def __init__(self,hp,Z_u,Y_test,Z_test,Beta,Beta0,u_test,BCy,BCz):
        '''
    ...
    def ls_train(self,cond,driver,sample_method): 
        ...
        for ii in range(z_nets): 
            ...
            for jj in range(y_nets): 
                ...
                E0 = getE1D(self.freq,self.h0,self.sig0,YZ[:,1])
                ...
                if ii ==0 and jj == 0:
                    ...
                    u_yy = u_yy0 + u_zz0
                    uu   = uu0*(Beta_**2)
                    bc0 = 2.0/(self.ub0_0-self.lb0_0)
                else: 
                    u_yy = (u_yy0/(bc0[0]**2)*(bc1[0]**2)) + (u_zz0/(bc0[1]**2)*(bc1[1]**2))
                    uu   = uu0*(Beta_**2) 
                # w_m = np.sum(np.abs(u_yy)+np.abs(uu),1,keepdims=True)
                u_y_all = np.concatenate((u_yy,uu),1)
                # w_m_r = np.linalg.norm(u_y_all.real,np.inf,axis=1,keepdims=True)
                # w_m_i = np.linalg.norm(u_y_all.imag,np.inf,axis=1,keepdims=True)
                # w_m = w_m_r + II*w_m_i
        ...
        for jj in range(y_nets): 
            for ii in range(z_nets):
                ...
                if ii > 0:
                    ...
                    if ii ==1:
                        u_pred_0, u_y_0 = self.net_interface(model,Y_inter0,lb,ub,"Z")
                        u_pred0 = np.copy(u_pred_0)
                        u_y0    = np.copy(u_y_0)
                        bc_inter0 = 2.0/(ub-lb)
                    else: 
                        u_y0    = u_y_0/bc_inter0[1]*bc1[1]
                ...
                if ii <z_nets-1:
                    ...
                    # u_pred1, u_y1 = self.net_interface(model,Y_inter1,lb,ub,"Z")
                    if ii ==0:
                        u_pred_1, u_y_1 = self.net_interface(model,Y_inter1,lb,ub,"Z")
                        u_pred1 = np.copy(u_pred_1)
                        u_y1    = np.copy(u_y_1)
                        bc_inter1 = 2.0/(ub-lb)
                    else: 
                        u_y1    = u_y_1/bc_inter1[1]*bc1[1]
                ...
                # Robin conditions
                    # u_pred, u_y = self.net_interface(model,self.Z_b[jj*y_inter:(jj+1)*y_inter,0:2],lb,ub,"Z")
                    # Beta_ = self.Beta[BCz[ii],BCy[jj]]
                    # uu = Beta_*u_pred
                    # u_y_all = np.concatenate((u_y,uu),1)
                    # wi_r = np.linalg.norm(u_y_all.real,axis=1,keepdims=True)
                    # wi_i = np.linalg.norm(u_y_all.imag,axis=1,keepdims=True)
                    # wi = w_i_r + II*w_i_i
                    # wi = np.linalg.norm(np.abs(u_y_all),axis=1,keepdims=True)
                    # # wi = np.linalg.norm(np.concatenate((u_y,uu),1),np.inf, 1,True)
                    # A[num_base+(row_pos+1)*y_inter:num_base+(row_pos+2)*y_inter,(col_pos)*num_nodes:(col_pos+1)*num_nodes] = (u_y + np.sqrt(-1.0*II)*uu)/wi
        ...
        for ii in range(z_nets): 
            for jj in range(y_nets):
                ...
                if jj>0 :# left
                    if jj ==1:
                        u_pred_0, u_y_0 = self.net_interface(model,Y_inter0,lb,ub,"Y")
                        u_pred0 = np.copy(u_pred_0)
                        u_y0    = np.copy(u_y_0)
                        bc_inter0 = 2.0/(ub-lb)
                    else: 
                        u_y0    = u_y_0/bc_inter0[0]*bc1[0]
                    if ii==1 and jj ==1:
...
###################################################################################################################################################
    ...
    def get_ex(self,Z):
        e0 = getE1D(self.freq, self.h0,self.sig0,Z)
        e1 = getE1D(self.freq, self.h0,self.sig0,Z+0.1)
        e_x = (e1-e0)/0.1
        return e0,e_x
    def get_e(self,Z):
        e0 = getE1D(self.freq, self.h0,self.sig0,Z[:,0])
        return e0.reshape(-1,1)*np.ones_like(Z)