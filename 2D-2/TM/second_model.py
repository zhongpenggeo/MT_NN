...
class second_model(object):
    def __init__(self,hp,Z_u,Y_test,Z_test,Beta,Beta_,u_test,E0_model,BCy,BCz):
        '''
        ...
        self.to_model(Z_u,Y_test,Z_test,Beta,Beta_,u_test,BCy,BCz)
    ...
    def ls_train(self,cond,driver,sample_method): 
        ...
        for ii in range(z_nets-1): 
            for jj in range(1,y_nets-1): 
                ...
                Beta_ = self.Beta_[BCz[ii],BCy[jj]]
                _,H = self.BC_p.compute_E_H(YZ)
                H0 = H[:,0:1]
                ...
                b[((ii*y_nets+jj))*num_points:((ii*y_nets+jj)+1)*num_points,:] = (-II*(Beta_**2)*H0)/w_m
                del u_yy,uu
        ...
        for jj in range(1,y_nets-1): 
            for ii in range(z_nets):
                ...
                if ii >0 and ii<z_nets-1:
                    ...
                    E,_ = self.BC_p.compute_E_H(Y_inter0)
                    E0 = E[:,1:2]
                    u_y0 = (u_y00)/sig
                    alpha_sig_0 = Beta_**2/self.omega/self.mu/sig
                    ...
                    b[num_base+(row_pos  )*y_inter:num_base+(row_pos+1)*y_inter,:] = (alpha_sig_1*E0-alpha_sig_0*E0)/w_i
                    ...
                    # w_r = 1.0#np.linalg.norm(u_pred,np.inf,axis=1,keepdims=True)
                ...
                if ii <z_nets-1:
                    ...
                    alpha_sig_1 = Beta_**2/self.omega/self.mu/sig
                    u_y1 = (u_y11)/sig
                    # u_y1 = u_y11/(Beta**2)
        ...
        for jj in [0, y_nets-1]:
            ...
            for ii in [0,z_nets-2,z_nets-1]:
                ...
                if ii ==0:
                    ...
                    sig = Beta**2/self.omega/self.mu
                    Beta_ = self.Beta_[BCz[ii],BCy[jj]]
                    # sig_ = Beta_**2/self.omega/self.mu
                    model = getattr(self,'model{}_{}'.format(z_nets-1,bc_lr))
                    lb = getattr(self,'lb{}_{}'.format(z_nets-1,bc_lr))
                    ub = getattr(self,'ub{}_{}'.format(z_nets-1,bc_lr))
                    u_pred1, u_y11 = self.net_interface(model,Y_inter1,lb,ub,"Z")
                    alpha_sig_1 = Beta_**2/self.omega/self.mu/sig
                    # E,_ = self.BC_p.compute_E_H(Y_inter1)
                    # E0 = E[:,1:2]
                    u_y1 = (u_y11)/sig
                    # u_y1 = u_y11/(Beta**2)
                    A[num_base+(row_pos+1)*y_inter:num_base+(row_pos+2)*y_inter,(col_pos)*num_nodes:(col_pos+1)*num_nodes] = u_pred1
    ...
    def to_model(self,Z_u,Y_test,Z_test,Beta,Beta_,u_test,BCy,BCz):
        self.Z_t = Z_u["Z_t"]
        self.Z_b = Z_u["Z_b"]
        self.Z_l = Z_u["Z_l"]
        self.Z_r = Z_u["Z_r"]
